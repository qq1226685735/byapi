...
import com.hh.util.ResourceUtil;
...
public class CodeAction extends HttpServlet {
    ...
    public void VerificationCode( HttpServletRequest request,
       ...
       /* String uid=TokenUtil.GetGUID();
        Cookie cookie = new Cookie("Code",uid);
        response.addCookie(cookie);
        jedisTool.setString(uid,resultCode);
        System.out.print(jedisTool.getString(uid));*/
        // 将认证码存入SESSION
        request.getSession().setAttribute("ucode", resultCode);
        // 图象生效
        graphics.dispose();
    ...
    /**
     * 描述：根据给定的数字生成不同的颜色
     * @param ，颜色模式是指使用三种基色：红、绿、蓝，通过三种颜色的调整得出其它各种颜色，这三种基色的值范围为0～255
     * @param颜色模式是指使用三种基色：红、绿、蓝，通过三种颜色的调整得出其它各种颜色，这三种基色的值范围为0～255
     * @return 描述：返回颜色
     */
